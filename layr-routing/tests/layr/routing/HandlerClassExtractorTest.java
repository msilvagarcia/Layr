...
public class HandlerClassExtractorTest {
	...
	public HandlerClassExtractorTest() {
		exceptionHandlerExtractor = HandlerClassExtractor.newInstance(ExceptionHandler.class);
		handlerClassExtractor = HandlerClassExtractor.newInstance(DataProvider.class);
	}
	...
	public void grantThatExtractDataAsExpected(){
		...
		Map<String, Class<ExceptionHandler>> handlers = exceptionHandlerExtractor.getRegisteredHandlers();
	...
	public void grantThatExtractDataProviderAsExpected(){
		...
		Map<String, Class<DataProvider>> handlers = handlerClassExtractor.getRegisteredHandlers();
		Assert.assertNotNull(handlers);
		Assert.assertEquals(1, handlers.size());
		Assert.assertEquals( RequestContextDataProvider.class, handlers.get(RequestContext.class.getCanonicalName()) );
	}
